...
def seg_background(img,segname, outname):
    ...
    level = filt.threshold_otsu(B) 
    # level = filt.threshold_isodata(B)
    # level = filt.threshold_li(B)
    # level = filt.threshold_mean(B)
    # level = filt.threshold_minimum(B)
    # level = filt.threshold_niblack(B)
    # level = filt.threshold_sauvola(B)
    # level = filt.threshold_yen(B)
    # mask = B > level #for old tau staining
    mask = B < level #for new tau staining
    se = morph.disk(5)
    mask2 = morph.opening(mask,se)
    # mask2 = mht.open(mask,se) 
    se2 = morph.disk(3)
    mask3 = morph.erosion(mask2, se2)
